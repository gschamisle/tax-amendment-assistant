"""신설 조문 검토 오프라인 테스트 — 조번호 재사용 충돌 시나리오 (조특법 제29조대).

DRAFT는 합성 예문이며, 제도명은 가칭(○○세액공제)이다. 조번호 29는 현행법에서
제127·132조 등이 옛 제29조대를 인용한다는 공개 사실(잔존 인용)을 검증하기 위한 것이다.
"""
...
DRAFT = (
    ...
    "「법인세법」 제4조에 따른 각 사업연도 소득이 있는 경우로서 대통령령으로 정하는 "
    "요건을 갖춘 경우에는 제29조의2에 따라 계산한 금액을 "
    "같은 법 제55조에 따른 세율을 적용한 산출세액의 범위에서 공제한다.\n"
    "제29조의2(공제금액의 계산) ① 법 제29조제1항의 공제금액은 기준금액에 "
    "공제율을 곱하여 계산한다."
)
